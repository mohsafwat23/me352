...
class ImagePublisher(Node):
    ...
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(TelloPose, 'video_frames', 2)
    ...
    def timer_callback(self):
        ...
        #self.tellpos.im
        if ret == True:
            self.tellpos.img = self.br.cv2_to_imgmsg(frame)
            self.tellpos.velocity.x = 0.
            self.tellpos.velocity.y = 0.
            self.tellpos.velocity.z = 0.

            # Publish the image.
            # The 'cv2_to_imgmsg' method converts an OpenCV
            # image to a ROS 2 image message
            #self.publisher_.publish(self.br.cv2_to_imgmsg(frame))
            self.publisher_.publish(self.tellpos)
